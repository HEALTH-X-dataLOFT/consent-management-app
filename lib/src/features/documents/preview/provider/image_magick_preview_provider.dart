...
class ImageMagickPreviewProvider extends PreviewProvider<ImageProvider> {
  @override
  bool canHandle({
  ...
  Future<ImageProvider> prepare(
    ...
    return MemoryImage(await _transform(data));
  }

  Future<Uint8List> _transform(Uint8List data) async {
    final wand = MagickWand.newMagickWand();

    try {
      if (!await wand.magickReadImageBlob(data)) throw "decoding error";
      if (!wand.magickSetImageFormat("PNG")) throw "invalid format";
      final transformed = await wand.magickGetImageBlob();
      if (transformed == null) throw "unable to transform image";
      return transformed;
    } finally {
